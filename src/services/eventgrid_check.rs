...
pub fn list_system_topics(sub: &str, rg: &str) -> Result<Vec<EgSystemTopic>, AzError> {
    ...
    let arr: Vec<serde_json::Value> = serde_json::from_str(&stdout)
        .map_err(|e| AzError::Other(format!("parse topics: {e}")))?;

    Ok(arr.iter().map(|v| EgSystemTopic {
...
pub fn list_topics(sub: &str) -> Result<Vec<EgTopic>, AzError> {
    ...
    Ok(arr.iter().map(|v| {
        ...
        let rg = v["id"].as_str().unwrap_or("")
            .split("/resourceGroups/").nth(1)
...
pub fn list_custom_topic_subscriptions(sub: &str, rg: &str, topic: &str) -> Result<Vec<EgSubscription>, AzError> {
    let mut cmd = az_command(&[
...
pub fn list_topic_subscriptions(sub: &str, rg: &str, topic: &str) -> Result<Vec<EgSubscription>, AzError> {
    let mut cmd = az_command(&[
...
pub fn fetch_all(sub: &str, rg: &str) -> Result<EgData, AzError> {
    ...
    for t in custom {
        let t_rg = if t.resource_group.is_empty() { rg } else { &t.resource_group };
    ...
    Ok(EgData { topics, system_topics })
}
...
fn parse_subscription(v: &serde_json::Value, topic_name: &str) -> EgSubscription {
    ...
    if let Some(s) = filter["subjectBeginsWith"].as_str() {
        if !s.is_empty() { filters.push(EgFilter { label: "Subject begins with".into(), value: s.into() }); }
    }
    if let Some(s) = filter["subjectEndsWith"].as_str() {
        if !s.is_empty() { filters.push(EgFilter { label: "Subject ends with".into(), value: s.into() }); }
    }
    ...
    if let Some(adv) = filter["advancedFilters"].as_array() {
        for f in adv {
            ...
            let values: Vec<String> = f["values"].as_array()
    ...
    let included_event_types = filter["includedEventTypes"].as_array()
        .map(|a| a.iter().filter_map(|v| v.as_str().map(String::from)).collect())
        .unwrap_or_default();
    ...
    let endpoint = dest["properties"]["endpointUrl"].as_str()
    ...
    EgSubscription {
        name: v["name"].as_str().unwrap_or("").into(),
        topic_name: if !topic_name.is_empty() { topic_name.into() } else {
            v["topic"].as_str().unwrap_or("").rsplit('/').next().unwrap_or("").into()
        },
        topic_type: v["topicType"].as_str().unwrap_or("").into(),
        endpoint_type,
        endpoint,
...
fn classify_error(stdout: &str, stderr: &str) -> AzError {
    ...
    if combined.contains("AADSTS") || combined.contains("az login") || combined.contains("refresh token") {
        ...
        AzError::Other(if stderr.is_empty() { stdout.to_string() } else { stderr.to_string() })
    }
}
