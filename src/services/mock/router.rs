...
impl Router {
    ...
    pub fn route(&self, incoming: &Incoming<'_>) -> Option<Arc<Endpoint>> {
        for r in &self.routes {
            if !r.method.eq_ignore_ascii_case(incoming.method) {
                continue;
            }
...
mod tests {
    ...
    use crate::services::mock::contract::{
        AuthKind, DiscoveryRef, Endpoint, RequestSpec, ResponseSpec,
    };
    ...
    #[test]
    fn match_exact_path() {
        let r = router_of(vec![make_endpoint("GET", "https://api.example.com/health")]);
        assert!(r
            .route(&Incoming {
                method: "GET",
                path: "/health"
    ...
    fn match_placeholder_segment() {
        let r = router_of(vec![make_endpoint(
            "POST",
            "https://x/api/bsfn/{methodName}/executeasync",
        )]);
        assert!(r
            .route(&Incoming {
                method: "POST",
                path: "/api/bsfn/Foo/executeasync"
            })
            .is_some());
        assert!(r
            .route(&Incoming {
                method: "POST",
                path: "/api/bsfn/Foo/Bar/executeasync"
            })
            .is_none());
    }
