            ...
            let detail = poll_until(*timeout_ms, &ctx.cancel, || async {
                match latest_terminal_run(workflow, floor, &claimed).await {
                    ...
                    Some((run_name, status)) => {
                        ...
                        Err(format!(
                            "run {run_name} finished {status}, expected {expect_status}{why}"
                        ))
                    }
                    ...
                    None => match workflows::list_runs(workflow).await {
                        ...
                        Err(e) => Err(e),
                        // A stateless workflow keeps no run history unless the
...
fn stateless_run_history_error(project_root: &Path, workflow: &str) -> Option<String> {
    ...
    let opted_in = settings["Values"]
        ...
        .map(|v| {
            v.iter().any(|(k, val)| {
                (k == &per_workflow || k == "Workflows.WorkflowRuntime.OperationOptions")
                    && val
                        .as_str()
                        .unwrap_or_default()
                        .contains("WithStatelessRunHistory")
            })
        })
        .unwrap_or(false);
    if opted_in {
...
mod tests {
    ...
    fn a_stateless_workflow_without_run_history_is_diagnosed() {
        ...
        std::fs::write(
            la.join("Pivot/workflow.json"),
            r#"{"kind":"Stateless","definition":{}}"#,
        )
        .unwrap();
