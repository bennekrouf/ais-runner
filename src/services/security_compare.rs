...
pub fn infer_env_from_group(group: &VarGroup) -> EnvTarget {
    ...
    EnvTarget {
        subscription: lookup(&[
            "subscriptionid", "azuresubscriptionid", "subscription",
            "armsubscriptionid", "subid",
        ]).unwrap_or_default(),
        ...
        cosmos_account: lookup(&[
            "cosmosaccountname", "cosmosdbaccountname", "cosmosdbaccount",
            "cosmosaccount", "cosmosname", "cosmosdbname",
            "cosmosdbsqlaccountname", "cosmosdbsqlaccount",
        ]),
        key_vault: lookup(&[
            "keyvaultname", "kvname", "keyvault", "azurekeyvaultname",
            "keyvaultaccountname",
        ]),
    }
}
...
fn collect_role_assignments(arr: &Value) -> Vec<RoleAssignment> {
    ...
    items.iter().map(|item| RoleAssignment {
...
pub fn fetch_cosmos_security(env: &EnvTarget) -> Result<CosmosSecurity, AzError> {
    let acct = env.cosmos_account.as_deref()
        .ok_or_else(|| AzError::Other("no cosmos account configured".into()))?;
    if env.subscription.is_empty() || env.resource_group.is_empty() {
        return Err(AzError::Other("missing subscription or resource group".into()));
    }

    let mut sec = CosmosSecurity { account_name: acct.to_string(), ..Default::default() };
    ...
    sec.key_metadata_write_enabled = show["disableKeyBasedMetadataWriteAccess"].as_bool().map(|b| !b);
    if let Some(arr) = show["ipRules"].as_array() {
        sec.ip_rules = arr.iter()
            .filter_map(|v| v["ipAddressOrRange"].as_str())
            .map(str::to_string).collect();
    }
    if let Some(arr) = show["virtualNetworkRules"].as_array() {
        sec.vnet_rules = arr.iter()
...
pub fn fetch_keyvault_security(env: &EnvTarget) -> Result<KeyVaultSecurity, AzError> {
    let vault = env.key_vault.as_deref()
        .ok_or_else(|| AzError::Other("no key vault configured".into()))?;
    if env.subscription.is_empty() || env.resource_group.is_empty() {
        return Err(AzError::Other("missing subscription or resource group".into()));
    }

    let mut kv = KeyVaultSecurity { vault_name: vault.to_string(), ..Default::default() };
...
pub fn resolve_principal(principal_id: &str) -> Option<String> {
    let attempts: &[&[&str]] = &[
        ...
        &["ad", "user",  "show", "--id",    principal_id, "--query", "displayName", "-o", "tsv"],
        &["ad", "group", "show", "--group", principal_id, "--query", "displayName", "-o", "tsv"],
    ];
