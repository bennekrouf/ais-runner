...
/// Where the work dir lived before it moved under `$HOME`. Only ever read, to
/// carry an existing Config.json forward — never written to again.
#[cfg(not(target_os = "windows"))]
const LEGACY_UNIX_WORK_DIR: &str = "/tmp/ais-sb-emulator";

/// Returns the working directory for compose + config files.
///
/// The path has to be visible to the Docker *VM*, not just to the host: the
/// emulator reads Config.json through a bind mount, and an unshared host path
/// mounts as an empty directory. The container then dies on startup with
/// `The configuration file 'ConfigFiles/Config.json' was not found` (exit 133)
/// and nothing ever binds :5672.
///
/// On macOS/Linux that means `$HOME` — the one host path every backend shares
/// by default. Docker Desktop shares `/Users`; Colima/Lima/Rancher mount `$HOME`
/// writable but NOT `/tmp`, which is why the old `/tmp/ais-sb-emulator` worked
/// on Docker Desktop and silently failed on every other runtime.
/// `std::env::temp_dir()` is worse still on macOS (`/var/folders/…`, shared by
/// no backend at all).
///
...
pub fn work_dir() -> PathBuf {
    if cfg!(target_os = "windows") {
        ...
        dirs::home_dir()
            .map(|h| h.join(".ais-runner").join("sb-emulator"))
            .unwrap_or_else(|| PathBuf::from("/tmp/ais-sb-emulator"))
    }
}

/// Carry a Config.json left in [`LEGACY_UNIX_WORK_DIR`] over to the new work dir
/// on first run, so queues added by hand through the UI survive the move.
/// Best-effort: no legacy file, or an unreadable one, just means a freshly
/// generated config.
fn migrate_legacy_work_dir(dir: &std::path::Path) {
    #[cfg(not(target_os = "windows"))]
    {
        let legacy = std::path::Path::new(LEGACY_UNIX_WORK_DIR).join("Config.json");
...
/// The emulator's ConfigFiles bind mount came up empty — the host work dir is
/// not shared with the Docker VM. The container aborts immediately (exit 133)
/// and this line is the only clue; without it the caller just waits out the
/// 4-minute bind timeout and reports a generic "didn't bind".
fn is_missing_config_crash(line: &str) -> bool {
    line.contains("ConfigFiles/Config.json") && line.contains("was not found")
}
...
fn write_compose_files(queues: &[(String, bool)]) -> Result<(PathBuf, Vec<String>), String> {
    ...
    migrate_legacy_work_dir(&dir);

    // Mount the whole directory, not a single file — single-file bind mounts are
    // unreliable on Docker Desktop for Mac and result in a null config inside the
    // container. The emulator reads Config.json from /ServiceBus_Emulator/ConfigFiles/.
...
pub fn handle_start(
    ...
    // Config.json, and read by the port poller below so an unshared work dir
    // fails in seconds with a real explanation instead of after 4 minutes.
    let mount_failed = Arc::new(std::sync::atomic::AtomicBool::new(false));
        ...
        Ok((stdout, stderr)) => {
            ...
            spawn(async move {
                ...
                while let Some((line, _)) = rx.recv().await {
                    ...
                    if is_missing_config_crash(&line)
                        && !mount_failed_w.swap(true, std::sync::atomic::Ordering::SeqCst)
                    {
                        push_mount(
                            format!(
                                "❌ The emulator's config mount is empty — Docker cannot see {}. \
                                 Docker Desktop: add the folder under Settings → Resources → File sharing. \
                                 Colima/Lima/Rancher: restart the VM with that path mounted, \
                                 e.g. `colima start --mount {}:w`.",
                                mount_dir, mount_dir,
                            ),
                            LogLevel::Error,
                        );
            ...
            spawn(async move {
                ...
                } else if mount_failed_r.load(std::sync::atomic::Ordering::SeqCst) {
                    // The stream task already printed the actionable message.
...
mod tests {
    ...
    /// Verbatim first line of the crash a non-shared work dir produces
    /// (emulator exit 133). If Microsoft rewords this, the fast-fail path
    /// silently degrades to the 4-minute timeout — hence the test.
    const EMPTY_MOUNT_CRASH: &str = "Unhandled exception. System.IO.FileNotFoundException: The configuration file 'ConfigFiles/Config.json' was not found and is not optional. The expected physical path was '/ServiceBus_Emulator/ConfigFiles/Config.json'.";

    #[test]
    fn empty_config_mount_crash_is_recognised() {
        assert!(is_missing_config_crash(EMPTY_MOUNT_CRASH));
    }

    #[test]
    fn ordinary_emulator_errors_are_not_mistaken_for_a_mount_failure() {
    ...
    fn work_dir_is_under_home_so_every_docker_backend_can_mount_it() {
        ...
        assert_ne!(dir, PathBuf::from(LEGACY_UNIX_WORK_DIR));
    }
}
