...
pub fn handle_start(
    ...
    spawn(async move {
        ...
        if tokio::net::TcpStream::connect(("127.0.0.1", COSMOS_API_PORT))
            .await
            .is_ok()
        {
            push(
                format!(
                    "Cosmos emulator already reachable on port {} — marking as running.",
                    COSMOS_API_PORT
                ),
                LogLevel::Ok,
            );
        ...
        push(
            format!("  Connection string for local.settings.json:"),
            LogLevel::Info,
        );
        push(
            format!("  CosmosDBConnection = {}", local_connection_string()),
            LogLevel::Info,
        );
        ...
        match run_result {
            Ok(Ok(out)) if out.status.success() => {
                push(
                    "Cosmos emulator container started — waiting for API to be ready…".into(),
                    LogLevel::Ok,
                );
            }
            Ok(Ok(out)) => {
        ...
        for attempt in 1..=20 {
            ...
            if attempt % 5 == 0 {
                push(
                    format!("Still waiting for Cosmos emulator… ({}/20)", attempt),
                    LogLevel::Info,
                );
            }
        }
        ...
        if ready {
            ...
            push(
                "❌ Cosmos emulator did not become ready in time. Check Docker logs.".into(),
                LogLevel::Error,
            );
            push(
...
pub fn handle_stop(
    ...
    let mut push = make_push(log_lines);
    push(
        format!("Stopping Cosmos emulator ({})…", CONTAINER_NAME),
        LogLevel::Warn,
    );
